# Laboratory Protocols

Welcome to our comprehensive laboratory protocols repository. This collection contains **37 protocols** organized across multiple disciplines, designed to ensure reproducibility and facilitate knowledge sharing in the research community.

## 📖 About This Repository

This repository contains detailed protocols that have been developed, tested, and optimized in our laboratory. However, some protocols have not been updated or been finished, yet! Those are marked in the README.md files in every subsejction as *under construction*. Each protocol includes:

- **Standardized metadata** with tags, categories, and update history
- **Clear step-by-step procedures** with timing estimates
- **Materials and equipment lists**
- **Troubleshooting notes** and best practices
- **Literature references** where applicable

**Usage Guidelines:**
- You are welcome to reference or adapt these protocols for your research
- Please consult with the authors and me before publishing work based on these protocols
- If you notice errors or have improvements, please let us know

## 🔍 Quick Navigation

| Category | Protocols | Description |
|----------|-----------|-------------|
| [Biophysics](#biophysics) | 3 | Biophysical characterization techniques |
| [Buffers](#buffers) | 2 | Buffer and media preparation protocols |
| [Chemistry](#chemistry) | 2 | Analytical chemistry and assay protocols |
| [Dna](#dna) | 4 | DNA manipulation, cloning, and mutagenesis protocols |
| [Molbio](#molbio) | 5 | Molecular biology techniques including transformation and cell preparation |
| [Protein](#protein) | 21 | Protein expression, purification, and analysis protocols |

## 📚 Protocol Index

### Biophysics

*Biophysical characterization techniques*

#### [Cyanide FTIR Probing](Biophysics/Cyanide-FTIR-Probing.md) (*under construction*)
**Authors:** Max | **Updated:** 2023-08-31

The idea is to probe the assembly of the active site of NiCODH by incubating the protein with CN and then trying to see ...

**Tags:** `biophysics`

---

#### [EPR](Biophysics/EPR_cryo_setUP.md) (*under construction*)
**Authors:** Max | **Updated:** 2024-10-11

---

#### [How To FTIR](Biophysics/How-To-FTIR.md) (*under construction*)
**Authors:** Max | **Updated:** 2023-10-12

A general protocol on how to operate the FTIR.

**Tags:** `biophysics`

---

### Buffers

*Buffer and media preparation protocols*

#### [Electrophoresis-Buffers](Buffers/Electrophoresis-Buffers.md)
**Authors:** Max | **Updated:** 2025-02-12

**Tags:** `buffers`

---

#### [M9 media ](Buffers/M9-media-B.md)
**Authors:** Briggi | **Updated:** unknown

**Tags:** `buffers`, `media`

---

### Chemistry

*Analytical chemistry and assay protocols*

#### [ICP-OES Protocol](Chemistry/ICP-OES-Protocol.md)
**Authors:** Max | **Updated:** 2025-03-24

ICP-OES is a great technique to determine the concentrations of Metal ions in solution. It takes about *45 min to set up...

**Tags:** `chemistry`, `analytical`, `ICP-OES`

---

#### [Iron Determination micro assay](Chemistry/Iron-Determination-microassay-B.md)
**Authors:** unknown, Briggi, Max | **Updated:** unknown

Iron determination assay for very small amounts of iron in a sample, eg protein.

**Tags:** `dna`

---

### Dna

*DNA manipulation, cloning, and mutagenesis protocols*

#### [DNA Fragment Isolation from Gel](DNA/DNA-Fragment-Isolation-from-Gel-B.md)
**Authors:** Briggi | **Updated:** unknown

## Material and Equipment

**Tags:** `dna`

---

#### [Megaprimer Mutagenesis](DNA/Megaprimer-Mutagenesis.md)
**Authors:** Kentaro Miyazaki, Notes from Max | **Updated:** 2023-08-30

Handy method to mutate a gene at the end and on the beginning a a 500to1000bp stretch!

---

#### [Restriction Enzyme Cloning](DNA/Restriction-Enzyme-Cloning.md)
**Authors:** Max, Maria | **Updated:** 2022-08-31

This guide will carry you trough the whole journey of cloning via restricion enzymes. This can be used, to change vector...

**Tags:** `dna`

---

#### [Site Directed Mutagenesis](DNA/Site-Directed-Mutagenesis.md)
**Authors:** Max | **Updated:** 2023-07-21 (not done yet)

How to design a primer for introducing a mutation in a gene via PCR. Followed by [[2004Zheng]].

**Tags:** `dna`

---

### Molbio

*Molecular biology techniques including transformation and cell preparation*

#### [Chemical Competent E. Coli](Molecular-Biology/Chemical-Competent-E-Coli.md)
**Authors:** Max | **Updated:** 2022-08-18

How to make chemical competent E. coli, with CaCl2. This was taken from "Current Protocols in Molecular Biology Unit 1.8...

**Tags:** `molbio`

---

#### [Chemically competent cell preperation](Molecular-Biology/Chemically-competent-cell-preparation-B.md)
**Authors:** Briggi, Max | **Updated:** unknown

## Material and Equipment

**Tags:** `molbio`

---

#### [Glycerol Stock](Molecular-Biology/Glycerol-Stock.md)
**Authors:** Maximilian Böhm | **Updated:** 2022-08-29

Tiny Protocol for making a glycerol stock of *E. coli*.

**Tags:** `molbio`

---

#### [Transformation of chemically competent Cells](Molecular-Biology/Transformation-of-chemically-competent-Cells.md)
**Authors:** Briggi, Max | **Updated:** 2022-10-28

"Transform: To put DNA into an organisms."

**Tags:** `molbio`

---

#### [Transformation of multiple Plasmids](Molecular-Biology/Transformation-of-multiple-Plasmids.md)
**Authors:** Max | **Updated:** 2023-11-10

## Material and Equipment

---

### Protein

*Protein expression, purification, and analysis protocols*

#### [96-Well Protein Expression and Activity Assay Protocol](Protein/96-well-Expression-and-Activity.md)
**Authors:** Max | **Updated:** 2024-08-15

This protocol outlines the steps for expressing CODHs in a 96-well plate format, which is ideal for large-scale screenin...

---

#### [Activation of FeFe Hydrogenase with Synthetic Cofactors](Protein/Activation-of-FeFe-Hydrogenase-with-Synthetic-Cofactors.md)
**Authors:** Princess, Max | **Updated:** unknown

FeFe Hydrogenases, when expressed heterologously in E. Coli without maturation enzymes, lack their active site. This is ...

**Tags:** `protein`

---

#### [Are BL21 deltaISCR or BL21 with pISC plasmid better for my protein?](Protein/deltaISCR-or-pISC.md) (*under construction*)
**Authors:** Maximilian Böhm | **Updated:** 2023-10-30

This is an experimental design (probably nested), to check the effect of IPTG, Iron source and strain on the effect of t...

**Tags:** `protein`, `expression`, `comparison`

---

#### [Bradford Microtube Assay](Protein/Bradford-Microtube-Assay.md)
**Authors:** Unknown | **Updated:** Unknown

SOP for protein concentration measurement when 280nm measurement is not available (e.g. due to FeS clusters). Suitable f...

**Tags:** `protein`

---

#### [CODH Expression](Protein/CODH-Expression.md)
**Authors:** Max, but inspiration taken from [here](https://pubs.acs.org/doi/full/10.1021/acsami.2c09547) and other sources | **Updated:** 2025-08-12

Here I summarised the general protocol for the expression of CODH in *E. coli*. There are different protocols out there ...

**Tags:** `protein`

---

#### [CODH Expression - Inoue](Protein/CODH-Expression-Inoue.md)
**Authors:** Max from [Inoue et al. (2011)](http://dx.doi.org/10.1271/bbb.110159) | **Updated:** 2023-02-13

How to express CODH II from C. hydrogenoformans in E Coli. An adapted protocol from [Inoue et al. (2011)](http://dx.doi....

**Tags:** `protein`

---

#### [CODH Purification Protocol](Protein/CODH-purification.md)
**Authors:** Max | **Updated:** 2024-09-22

This is the general purification protocol for CODHs. It took 2.5 years to optimize in our lab. It is very important to b...

**Tags:** `protein`

---

#### [CODH activity - CO oxidation](Protein/CODH-activity-CO-oxidation.md)
**Authors:** Max | **Updated:** 2023-03-15

With this assay you can monitor the oxidation of CO (or any substrate), indirectly by the subsequence reduction of methy...

**Tags:** `protein`

---

#### [CODH activity - CO2 Reduction Assay](Protein/CODH-activity-CO2-reduction.md)
**Authors:** Max | **Updated:** 2025-03-13

The reduction of CO2 is a characteristic properties of CODHs. Here you find the procedure to determine the CO2 reduction...

**Tags:** `protein`

---

#### [CODH-activity-CO2-reduction-mDQ](Protein/CODH-activity-CO2-reduction-mDQ.md)
**Authors:** Max | **Updated:** 2025-02-12

We are unsure, if tested CODH are unactive towards CO2 reduction beacause of their inherent properties, or because the d...

---

#### [General CsdA Expression and Purification](Protein/General-CsdA-Expression-and-Purification-B.md)
**Authors:** Briggi | **Updated:** unknown

**Tags:** `protein`

---

#### [Lysis Buffer Screening](Protein/Lysis-Buffer-Screening.md) (*under construction*)
**Authors:** Max | **Updated:** 2022-12-12

This is a short Method on how to screen quickly for the optimal lysis buffer.

**Tags:** `buffer`, `protein`

---

#### [NH2OH Reduction Assay](Protein/NH2OH-Reduction-Assay.md)
**Authors:** Max, and from here https://journals.asm.org/doi/10.1128/JB.184.21.5898-5902.2002 and https://www.ncbi.nlm.nih.gov/pubmed/35133707 | **Updated:** 2022-03-02

The ability of Hybrid Cluster Protein and Carbon Monoxide Dehydrogenases to reduce Hydroxylamine to Ammonia and Water ha...

**Tags:** `protein`

---

#### [NO Reduction Assay](Protein/NO-Reduction-Assay.md) (*under construction*)
**Authors:** ## Introduction | **Updated:** 2023-08-30

Its a mess... not an easy assay...

---

#### [Nickel Insertion Protocol](Protein/Nicke-Insertion-Protocol.md) (*under construction*)
**Authors:** ## Introduction | **Updated:** 2023-11-17

It has been shown that Nickel can be inserted into CODHs in vitro by incubating with Nickel and reductant for some time....

**Tags:** `protein`

---

#### [PFE - CODH](Protein/PFE-CODH.md) (*under construction*)
**Authors:** Max, Tipps from Patricia | **Updated:** 2023-07-11

[[Protein Film Electrochemistry]] PFE is a very powerfull tool.

---

#### [Protein pDot Interaction Electrophoresis](Protein/Protein-pDot-Interaction-Electrophoresis.md) 
**Authors:** Mariia, Max | **Updated:** 2023-04-26

To determine the interaction between proteins and pdots a simple electrophoresis experiment can be conducted. It has bee...

---

#### [Reconstitution of FeS Cluster](Protein/Reconstitution-of-FeS-Cluster.md)
**Authors:** Princess, Max | **Updated:** unknown

During the expression of Iron Sulfer (FeS) Cluster containing proteins, the insufficent assambly of them is a common pro...

**Tags:** `protein`

---

#### [SDS-PAGE Gels](Protein/SDS-PAGE.md)
**Authors:** Unknown | **Updated:** Unknown

**Tags:** `protein`

---

#### [SciFest - Antenna Protein Extraction](Protein/SciFest-Antenna-Protein.md)
**Authors:** Max | **Updated:** 2023-09-06

*Taken from a protocol written in a undergrad lab by me.* Phycobiliproteins or chromoproteins are the main part of the l...

**Tags:** `protein`, `education`

---

#### [Size Exclusion Chromatography](Protein/SEC.md) (*under construction*)
**Authors:** Max | **Updated:** 2023-06-14

Under CONSTURCTION

---

## 🛠️ Contributing

### Protocol Template
For consistency, new protocols should follow our standardized format:

```yaml
---
tags:
  - protocol
  - [category]
  - [technique]
category: [category]
---
# Protocol Title

**Last Updated**: YYYY-MM-DD
**Authors**: Your Name

## Introduction
Brief description and purpose...

## Materials and Equipment
- Required materials
- Equipment needed

## Procedure
1. Step-by-step instructions
2. Include timing estimates
3. Note critical steps

## Notes
- Troubleshooting tips
- Alternative approaches
- Safety considerations
```

### Metadata Standards
- Use consistent tagging with relevant keywords
- Include accurate update dates and author information
- Add brief but informative descriptions
- Categorize appropriately for better discoverability

## 📝 Writing Guidelines

We follow specific writing guidelines to ensure clarity and consistency. See our [Writing Guide](Writing-Guide.md) for detailed style recommendations including:

- Plain language usage
- Proper scientific formatting
- Unit notation standards
- Reference formatting

## 🔗 Related Resources

- [Writing Guide](Writing-Guide.md) - Style and formatting guidelines
- Protocol optimization tips and best practices (*under construction*)
- Equipment booking and lab safety information (*under construction*)

## 📞 Contact

For questions, suggestions, or collaboration opportunities, please contact the protocol authors directly or reach out to the repository maintainers.

---

*This index is automatically updated when protocols are modified. Last generated: {total_protocols} protocols indexed.*
